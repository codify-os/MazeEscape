...
public class BigBoss extends Enemy {
    ...
    private static final int SPECIAL_DETECTION_RANGE = 16 * 48; // increased for special mode
    ...
    public void update() {
        ...
        // ---- MODE SWITCHING ----
        if (mode == BossMode.NORMAL && now - modeStartTime >= 20000) {
            enterSpecialMode();
        }
    ...
    private void updateNormalMode() {
        if (distanceToPlayer() < NORMAL_DETECTION_RANGE) {
            chasePlayer();
            //spawnMinion();
            spawnBomb();
        }
